...
import { cron, ops, reputation } from '@rivlayx/core';
...
export async function GET(request: Request) {
  ...
  const db = getDb();
  const locked = await ops.recordCronRun(db, 'reputation', () =>
    cron.withAdvisoryLock(db, cron.CRON_LOCK_KEYS.reputation, async () =>
